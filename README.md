...
Bundled `ValidatorFn`s composing cleanly with Angular's built-in `Validators`: `noWhitespace`, `hexColor`, `url`, `cardNumber` (Luhn), `cvc`, `iban` (mod-97), `match` (sibling control), `oneOf`, `minDate`, `maxDate`. See [docs](https://ngwr.dev/validators).
