...
- **`projects/lib/`** — the published package (`ngwr`). Almost every subfolder is
  a **tree-shakable secondary entry point** consumed as `ngwr/<name>` — **177**
  ...
  points, and forty-nine are nested — `ngwr/i18n/{en,ru}`,
  `ngwr/icon/adapters/{lucide,feather}` and the CDK test harnesses, which now cover
  **forty-five** entry points: the form controls (`button`, `input`, `textarea`,
  ...
  `lightbox`, `tour`, `calendar`, `event-calendar`, `window`, `image-cropper` and
  the markdown renderer — each at `ngwr/<name>/testing`, 77 harness classes in total — `WrCalendarDayHarness` being
...
library where an internal tool exists. The catalog is large (177 entry points):
...
The catalog is large (177 entry points) and **deliberately consolidated** —
