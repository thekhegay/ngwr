...
- **`projects/lib/`** — the published package (`ngwr`). Almost every subfolder is
  a **tree-shakable secondary entry point** consumed as `ngwr/<name>` — **137**
  ...
  points, and eleven are nested — `ngwr/i18n/{en,ru}`,
  `ngwr/icon/adapters/{lucide,feather}` and the CDK test harnesses at
  `ngwr/{button,input,checkbox,switch,select,dialog,toast}/testing`. Built with
...
library where an internal tool exists. The catalog is large (137 entry points):
...
The catalog is large (137 entry points) and **deliberately consolidated** —
