...
- **`projects/lib/`** — the published package (`ngwr`). Almost every subfolder is
  a **tree-shakable secondary entry point** consumed as `ngwr/<name>` — **142**
  ...
  points, and sixteen are nested — `ngwr/i18n/{en,ru}`,
  `ngwr/icon/adapters/{lucide,feather}` and the CDK test harnesses at
  `ngwr/{button,input,checkbox,switch,select,dialog,toast,date-picker,dropdown,popover,drawer,table}/testing`. Built with
...
library where an internal tool exists. The catalog is large (142 entry points):
...
The catalog is large (142 entry points) and **deliberately consolidated** —
