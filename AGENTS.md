...
- **`projects/lib/`** — the published package (`ngwr`). Almost every subfolder is
  a **tree-shakable secondary entry point** consumed as `ngwr/<name>` — **172**
  ...
  points, and forty-four are nested — `ngwr/i18n/{en,ru}`,
  `ngwr/icon/adapters/{lucide,feather}` and the CDK test harnesses, which now cover
  **forty** entry points: the form controls (`button`, `input`, `textarea`,
  ...
  `lightbox` and the markdown renderer — each at `ngwr/<name>/testing`, 70 harness
  classes in total.
...
library where an internal tool exists. The catalog is large (172 entry points):
...
The catalog is large (172 entry points) and **deliberately consolidated** —
