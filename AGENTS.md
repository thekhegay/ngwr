...
- **`projects/lib/`** — the published package (`ngwr`). Almost every subfolder is
  a **tree-shakable secondary entry point** consumed as `ngwr/<name>` — **170**
  ...
  points, and forty-two are nested — `ngwr/i18n/{en,ru}`,
  `ngwr/icon/adapters/{lucide,feather}` and the CDK test harnesses, which now cover
  **thirty-eight** entry points: the form controls (`button`, `input`, `textarea`,
  ...
  markdown renderer — each at `ngwr/<name>/testing`, 67 harness classes in total.
...
library where an internal tool exists. The catalog is large (170 entry points):
...
The catalog is large (170 entry points) and **deliberately consolidated** —
