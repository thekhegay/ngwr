...
- **`projects/lib/`** — the published package (`ngwr`). Almost every subfolder is
  a **tree-shakable secondary entry point** consumed as `ngwr/<name>` — **171**
  ...
  points, and forty-three are nested — `ngwr/i18n/{en,ru}`,
  `ngwr/icon/adapters/{lucide,feather}` and the CDK test harnesses, which now cover
  **thirty-nine** entry points: the form controls (`button`, `input`, `textarea`,
  ...
  `stepper`, `carousel`, `pagination`, `collapse`, `transfer`), `splitter`, `speed-dial` and
  the markdown renderer — each at `ngwr/<name>/testing`, 69 harness classes in total.
...
library where an internal tool exists. The catalog is large (171 entry points):
...
The catalog is large (171 entry points) and **deliberately consolidated** —
