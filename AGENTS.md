...
- **`projects/lib/`** — the published package (`ngwr`). Almost every subfolder is
  a **tree-shakable secondary entry point** consumed as `ngwr/<name>` — **173**
  ...
  points, and forty-five are nested — `ngwr/i18n/{en,ru}`,
  `ngwr/icon/adapters/{lucide,feather}` and the CDK test harnesses, which now cover
  **forty-one** entry points: the form controls (`button`, `input`, `textarea`,
  ...
  `lightbox`, `tour` and the markdown renderer — each at `ngwr/<name>/testing`, 71
  harness classes in total.
...
library where an internal tool exists. The catalog is large (173 entry points):
...
The catalog is large (173 entry points) and **deliberately consolidated** —
