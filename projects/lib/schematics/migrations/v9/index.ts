 ...
 * bound group.
 *
 * A quoted value is consumed only as a PAIR, and both halves of that matter.
 * Stepping OVER it is what a plain `[^>]*?` got wrong for two majors: a `>`
 * inside a binding expression looks like the end of the tag, so
 * `<wr-checkbox [disabled]="n > 0" value="a">` kept its `value` while the same
 * attributes in the other order moved. The leftover is SILENT — a static
 * `value="a"` that matches no input is an ordinary DOM attribute,
 * `strictTemplates` says nothing, and every checkbox in the group keeps the
 * default identity `null`, so they all toggle together. That is the exact defect
 * this migration exists to prevent, reintroduced by the migration itself.
 *
 * Never consuming a LONE quote is the half the first fix missed. It spelled the
 * fallback `[^>]`, which matches a quote too, so the engine could backtrack into
 * a single-quoted span starting at the apostrophe of `label="it's"` and running
 * past `/>` into the next element: `<wr-checkbox label="it's" />` followed by
 * `<wr-select [x]="'y'" value="z" />` renamed the SELECT's form model. Now a rename
 * stays inside one open tag whenever every quote in it opens or closes a value;
 * an UNQUOTED value holding a quote (`title=it's`) breaks that, and is skipped.
 * `migration-v14`'s `IN_TAG` carries the same pattern and the longer account.
 */
