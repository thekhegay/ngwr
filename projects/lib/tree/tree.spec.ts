...
/**
 * A flip of the reading direction while the overlay tree is OPEN.
 *
 ...
 * again. This component makes the split visible from both sides: the arrow-key
 * mapping above reads `Directionality` LIVE, so a panel left behind on the old
 * direction took ArrowLeft as "open" while still drawing its indent rightward.
 *
 ...
 */
describe('WrTree in overlay mode follows a direction flip while open', () => {
  let fixture: ReturnType<typeof TestBed.createComponent<OverlayHost>>;

  const trigger = (): HTMLElement =>
    (fixture.nativeElement as HTMLElement).querySelector<HTMLElement>('[role="combobox"]')!;
  const pane = (): HTMLElement | null => document.querySelector<HTMLElement>('.wr-tree-overlay');
  /**
