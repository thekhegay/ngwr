...
/** Image-driven logo entry. */
export interface WrLogoLoopImage {
  readonly src: string;
  readonly alt?: string;
...
/** Template-driven logo entry — `node` is rendered via `*ngTemplateOutlet`. */
export interface WrLogoLoopNode {
...
export type WrLogoLoopItem = WrLogoLoopImage | WrLogoLoopNode;

function isNode(item: WrLogoLoopItem): item is WrLogoLoopNode {
  return 'node' in item;
}
...
/**
 * Endless horizontal logo carousel. Auto-duplicates the logo list until
 * it covers the container width, then animates the track with a smooth
...
@Component({
  ...
  host: {
    class: 'wr-logo-loop',
    '[class.wr-logo-loop--fade]': 'fadeOut()',
    '[class.wr-logo-loop--scale-hover]': 'scaleOnHover()',
    '[style.--wr-logo-loop-gap.px]': 'gap()',
    '[style.--wr-logo-loop-height.px]': 'logoHeight()',
    '[style.--wr-logo-loop-fade-color]': 'fadeOutColor() || null',
    '[attr.role]': '"region"',
    '[attr.aria-label]': 'ariaLabel()',
  },
})
export class WrLogoLoop {
  /** Logos to display. Each item is either an image source or a TemplateRef node. */
  readonly logos = input.required<readonly WrLogoLoopItem[]>();
  ...
  readonly logoHeight = input(28, { transform: num(28) });

  /** Gap between logos in pixels. @default 32 */
  ...
  /** Scale individual logos up on hover. @default false */
  readonly scaleOnHover = input(false, { transform: coerceBooleanProperty });

  /** Accessible label for the carousel region. @default 'Partner logos' */
  ...
  constructor() {
    ...
    // Re-measure on input changes that affect layout (logos, gap, height).
    effect(() => {
      this.logos();
      this.gap();
      this.logoHeight();
  ...
  private measure(): void {
    ...
    const seqEl = hostEl.querySelector<HTMLElement>('.wr-logo-loop__list');
    if (!seqEl) return;
