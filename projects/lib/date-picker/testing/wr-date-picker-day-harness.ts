...
/**
 * Test harness for one day cell of the calendar inside a date-picker popup.
 *
 * Reached through `WrDatePickerHarness.getDay()` / `getDays()` (or the range
 * picker's), which scope it to ONE picker's panel — the cells are in the overlay
 * container, so a bare query would answer with whichever picker opened first.
 *
 ...
 */
export class WrDatePickerDayHarness extends ComponentHarness {
  static hostSelector = '.wr-calendar__day';

  /** Build a predicate that narrows the query. */
  static with(options: WrDatePickerDayHarnessFilters = {}): HarnessPredicate<WrDatePickerDayHarness> {
    return new HarnessPredicate(WrDatePickerDayHarness, options)
   ...
   * which is why {@link WrDatePickerHarness.getDay} skips them.
   */
