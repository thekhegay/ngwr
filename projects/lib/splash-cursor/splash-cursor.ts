...
export class WrSplashCursor {
  ...
  readonly simResolution = input(128, { transform: numAttr(128) });

  /** Dye texture resolution — the visible detail. @default 1440 */
  readonly dyeResolution = input(1440, { transform: numAttr(1440) });

  /** How fast the dye fades, higher = shorter trails. @default 3.5 */
  readonly densityDissipation = input(3.5, { transform: numAttr(3.5) });

  /** How fast motion settles. @default 2 */
  readonly velocityDissipation = input(2, { transform: numAttr(2) });

  /** Pressure retained between frames, 0..1. @default 0.1 */
  readonly pressure = input(0.1, { transform: numAttr(0.1) });

  /** Jacobi iterations for the pressure solve. @default 20 */
  readonly pressureIterations = input(20, { transform: numAttr(20) });

  /** Vorticity confinement — swirliness. @default 3 */
  readonly curl = input(3, { transform: numAttr(3) });

  /** Splat size. @default 0.2 */
  readonly splatRadius = input(0.2, { transform: numAttr(0.2) });

  /** Velocity a pointer move injects. @default 6000 */
  readonly splatForce = input(6000, { transform: numAttr(6000) });
  ...
  readonly colorUpdateSpeed = input(10, { transform: numAttr(10) });
