...
- [ ] **A2. CDK test harnesses** (L, soft-blocked on A1) — ship
      ...
      **Shipped: 42 nested entry points, ~1022 harness specs** — every form control
      ...
      `tour`, `calendar` and `markdown` — nested, so a spec
    ...
    **Still uncovered:** event-calendar, window, image-cropper, virtual-scroll,
    plus the charts and the animation set. Every CONTROL and every OVERLAY has one now, so what is left is
